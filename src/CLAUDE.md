...
## API Layer (lib/api/)
`lib/api/client.ts` — `apiFetch` wrapper (server-only, handles auth header, timeout, error normalization) · `lib/api/[domain].ts` — domain modules wrapping `apiFetch` with `cache()` and Zod validation of upstream responses · `lib/auth/session.ts` — `getServerSession()` / `requireSession()` · `lib/schemas/` — Zod schemas (no `server-only`, shared client+server)
