...
> 24 rules. Full playbook: `docs/security-playbook.md` (100+ rules with IDs).
...
02 — Never AI-built auth. Use battle-tested providers. (AUTH-02)
03 — API keys in env vars only. Never hardcode. (AUTH-03)
...
## API & Access Control
09 — Row-Level Security from day one. (DB-01)
10 — Remove all `console.log` before production. (API-04)
11 — CORS restricted to allow-listed production domain. (API-02)
...
16 — Lock storage per-user. Users access only their files. (UPLOAD-04)
17 — Validate uploads by magic bytes, not extension. (UPLOAD-01)
18 — Verify webhook signatures before processing. (PAY-01, INFRA-05)

## Other Rules
19 — Server-side permissions only. UI checks are cosmetic. (AUTHZ-01, LOG-03)
20 — Log critical actions: deletions, role changes, exports. (LOG-01)
21 — Build real account deletion flows. GDPR compliance. (LOG-02)
22 — Automate backups then test restores. (INFRA-06)
23 — Test and production fully separate. (INFRA-03)
24 — Test webhooks never touch production systems. (INFRA-04)

## Pre-Commit
No secrets · No console.log · Inputs validated · Queries parameterised · Auth on endpoints · Rate limiting · Generic errors · No dynamic code evaluation · Upload magic bytes · Mass assignment blocked

## Pre-Deploy
npm audit clean · Security headers (HSTS/CSP) · HTTPS only · Env vars for secrets · Webhook sigs verified · Env separated · DDoS protection · AI cost caps
